...
/**/class MagicMoodLamp extends Effect {  //VisualCube cube = new VisualCube(this);
  ...
  for (int i = 0; i < VisualCube.width; i++) {
    for (int j = 0; j < VisualCube.height; j++) {
      for (int k = 0; k < VisualCube.depth; k++) {
        int rn = int(xn * (1f - r * i/VisualCube.width)  * (VisualCube.colors-1));
        int gn = int(yn * (1f - g * j/VisualCube.height) * (VisualCube.colors-1));
        int bn = int(zn * (1f - b * k/VisualCube.depth)  * (VisualCube.colors-1));
